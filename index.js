...
const setupDatabase = async () => {
    ...
    try {
        await client.query(`CREATE TABLE IF NOT EXISTS Users (id SERIAL PRIMARY KEY, name TEXT NOT NULL, username TEXT NOT NULL UNIQUE, email TEXT NOT NULL UNIQUE, password TEXT NOT NULL, role TEXT NOT NULL CHECK(role IN ('user', 'agent', 'admin')) DEFAULT 'user')`);
        console.log("Users table is ready.");
        await client.query(`CREATE TABLE IF NOT EXISTS Tickets (id SERIAL PRIMARY KEY, title TEXT NOT NULL, description TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'open' CHECK(status IN ('open', 'in_progress', 'closed')), priority TEXT NOT NULL DEFAULT 'medium' CHECK(priority IN ('low', 'medium', 'high')), created_by_user_id INTEGER REFERENCES Users(id) ON DELETE SET NULL, assigned_to_user_id INTEGER REFERENCES Users(id) ON DELETE SET NULL, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(), due_date TIMESTAMPTZ, version INTEGER DEFAULT 1)`);
        console.log("Tickets table is ready.");
        await client.query(`CREATE TABLE IF NOT EXISTS Comments (id SERIAL PRIMARY KEY, content TEXT NOT NULL, ticket_id INTEGER REFERENCES Tickets(id) ON DELETE CASCADE, user_id INTEGER REFERENCES Users(id) ON DELETE SET NULL, created_at TIMESTAMPTZ DEFAULT NOW())`);
        console.log("Comments table is ready.");
        await client.query(`CREATE TABLE IF NOT EXISTS TicketActions (id SERIAL PRIMARY KEY, ticket_id INTEGER NOT NULL REFERENCES Tickets(id) ON DELETE CASCADE, user_id INTEGER REFERENCES Users(id) ON DELETE SET NULL, action TEXT NOT NULL, details TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
...
app.patch('/api/users/:id', async (req, res) => {
    ...
    const params = [];
    let paramCount = 1;
    if (name) { fieldsToUpdate.push(`name = $${paramCount++}`); params.push(name); }
    if (username) { fieldsToUpdate.push(`username = $${paramCount++}`); params.push(username); }
    if (email) { fieldsToUpdate.push(`email = $${paramCount++}`); params.push(email); }
    if (password) {
        try {
            const hashedPassword = await bcrypt.hash(password, 10);
...
app.get('/api/tickets', async (req, res) => {
    ...
    let baseSql = `SELECT t.id, t.title, t.description, t.status, t.priority, t.created_at, t.updated_at, t.due_date, t.version, u.name as creator_name, au.name as assigned_agent_name, t.created_by_user_id, t.assigned_to_user_id FROM Tickets t LEFT JOIN Users u ON t.created_by_user_id = u.id LEFT JOIN Users au ON t.assigned_to_user_id = au.id`;
    ...
    else if (role === 'agent' && userId) { whereClauses.push(`(t.assigned_to_user_id = $${paramIndex++} OR t.status = 'open')`); params.push(userId); }
    ...
    if (search) { const searchTerm = `%${search}%`; whereClauses.push(`(t.title ILIKE $${paramIndex++} OR t.description ILIKE $${paramIndex++} OR u.name ILIKE $${paramIndex++})`); params.push(searchTerm, searchTerm, searchTerm); }
    if (whereClauses.length > 0) { const whereString = ' WHERE ' + whereClauses.join(' AND '); baseSql += whereString; countSql += whereString; }
    ...
    try {
        ...
        const countResult = await pool.query(countSql, params.slice(0, paramIndex - 3));
...
app.get('/api/tickets/:id', async (req, res) => {
    ...
    const sql = `
        ...
        WHERE t.id = $1`;
    try {
        // MODIFICATION IS HERE: Convert the 'id' string to a number
...
app.patch('/api/tickets/:id', async (req, res) => {
    ...
    try {
        ...
        if (current_version !== existingTicket.version) { return res.status(409).json({ error: 'Conflict: Ticket has been updated by someone else. Please refresh and try again.' }); }
        let fieldsToUpdate = [];
        const params = [];
        const actions = [];
        let paramCount = 1;
        if (status && status !== existingTicket.status) { fieldsToUpdate.push(`status = $${paramCount++}`); params.push(status); actions.push({ action_type: 'status_changed', details: `status: ${existingTicket.status} -> ${status}` }); }
        if (priority && priority !== existingTicket.priority) { fieldsToUpdate.push(`priority = $${paramCount++}`); params.push(priority); actions.push({ action_type: 'priority_changed', details: `priority: ${existingTicket.priority} -> ${priority}` }); }
        let actualAssignedToUserId = assigned_to_user_id === '' ? null : assigned_to_user_id;
        if (actualAssignedToUserId !== existingTicket.assigned_to_user_id) { fieldsToUpdate.push(`assigned_to_user_id = $${paramCount++}`); params.push(actualAssignedToUserId); }
        if (fieldsToUpdate.length === 0) { return res.status(400).json({ error: 'No meaningful fields to update provided.' }); }
        ...
        if (updateResult.rowCount === 0) { return res.status(404).json({ error: 'Ticket not found or no changes made.' }); }
...
app.get('/api/tickets/:id/comments', async (req, res) => {
    const { id: ticket_id } = req.params;
    const sql = `SELECT c.id, c.content, c.created_at, u.name as author_name FROM Comments c JOIN Users u ON c.user_id = u.id WHERE c.ticket_id = $1 ORDER BY c.created_at ASC`;
    try {
...
app.get('/api/tickets/actions/:id', async (req, res) => {
    const { id: ticket_id } = req.params;
    const sql = `SELECT ta.id, ta.action, ta.details, ta.created_at, u.name as actor_name FROM TicketActions ta LEFT JOIN Users u ON ta.user_id = u.id WHERE ta.ticket_id = $1 ORDER BY ta.created_at ASC`;
    try {
        const result = await pool.query(sql, [ticket_id]);
...
app.delete('/api/comments/:id', async (req, res) => {
    ...
    if (!adminId) { return res.status(400).json({ error: 'Admin ID is required for verification' }); }
    try {
        const userResult = await pool.query('SELECT role FROM Users WHERE id = $1', [adminId]);
        const user = userResult.rows[0];
        if (!user) { return res.status(400).json({ error: 'Invalid Admin ID or user not found.' }); }
        if (user.role !== 'admin') { return res.status(403).json({ error: 'Forbidden: Only admins can delete comments' }); }
